...
class TestStream < Minitest::Test
  ...
  def test_streams_thinking_from_reasoning_field
    ...
    assert_equal %i[start thinking_start thinking_delta thinking_delta thinking_end done], types(events)
  ...
  def test_thinking_prefers_first_nonempty_reasoning_field
    events, = collect([
      { "choices" => [{ "delta" => { "reasoning_content" => "A", "reasoning" => "A" } }] },
      text_chunk(nil, finish_reason: "stop")
    ])
  ...
  def test_streams_tool_call_assembled_across_chunks
    ...
    assert_equal %i[start toolcall_start toolcall_delta toolcall_delta toolcall_end done], types(events)
  ...
  def test_model_captured_from_chunk
    _events, acc = collect([
      { "model" => "gpt-4o-mini-2024", "choices" => [{ "delta" => { "content" => "x" } }] },
      text_chunk(nil, finish_reason: "stop")
    ])
  ...
  def test_partial_snapshots_are_independent_across_deltas
    events, = collect([text_chunk("one"), text_chunk("two"), text_chunk(nil, finish_reason: "stop")])
