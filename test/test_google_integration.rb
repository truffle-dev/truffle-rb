...
# End-to-end test against the real Gemini Generative Language API. Skipped unless
# GEMINI_API_KEY is set, so the default `rake test` stays hermetic and offline.
# Run it with a key present to verify the full round-trip: prompt -> model
# requests a tool -> Truffle runs it -> model answers with the result.
class TestGoogleIntegration < Minitest::Test
  def setup
    return unless ENV["GEMINI_API_KEY"].to_s.empty?

    skip "set GEMINI_API_KEY to run the live Google test"
  end
